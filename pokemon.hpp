...
public:
	PokemonAbility() {
		...
		m_ability_map[_T( "もふもふ(炎被弾)" )] = ABILITY_FLUFFY_FLARE; // 各ポケモンに情報持たせるようにしたから、攻撃側と防御側でそれぞれ作用するやつも分けなくても良いと思う
		m_ability_map[_T( "フレンドガード" )] = ABILITY_FRIENDGUARD;
		m_ability_map[_T( "氷の鱗粉" )] = ABILITY_ICE_SCALES;
		m_ability_map[_T( "パンクロック(攻撃側)" )] = ABILITY_PUNKROCK_ATK;
		m_ability_map[_T( "パンクロック(防御側)" )] = ABILITY_PUNKROCK_DEF;
		m_ability_map[_T( "ファントムガード" )] = ABILITY_SHADOWSHIELD;
		m_ability_map[_T( "マルチスケイル" )] = ABILITY_MULTISCALE;
		m_ability_map[_T( "もふもふ(接触技)" )] = ABILITY_FLUFFY_DIRECT;
		...
		m_ability_map[_T( "遠隔" )] = ABILITY_LONGREACH;
		...
		m_ability_map[_T( "ノー天気" )] = ABILITY_CLOUDNINE;
		m_ability_map[_T( "硬い爪" )] = ABILITY_TOUGHCLAWS;
		m_ability_map[_T( "頑丈顎" )] = ABILITY_STRONGJAW;
		m_ability_map[_T( "肝っ玉" )] = ABILITY_SCRAPPY;
		m_ability_map[_T( "強運" )] = ABILITY_SUPERLUCK;
		m_ability_map[_T( "激流" )] = ABILITY_TORRENT;
		m_ability_map[_T( "新緑" )] = ABILITY_OVERGROW;
		m_ability_map[_T( "猛火" )] = ABILITY_BLAZE;
		m_ability_map[_T( "変幻自在" )] = ABILITY_PROTEAN;
		m_ability_map[_T( "リベロ" )] = ABILITY_LIBERO;
		m_ability_map[_T( "五里霧中" )] = ABILITY_GORILLATACTICS;
		m_ability_map[_T( "根性" )] = ABILITY_GUTS;
		m_ability_map[_T( "サンパワー" )] = ABILITY_SOLARPOWER;
		m_ability_map[_T( "スキルリンク" )] = ABILITY_SKILLLINK;
		m_ability_map[_T( "水泡" )] = ABILITY_WATERBUBBLE;
		m_ability_map[_T( "捨て身" )] = ABILITY_RECKLESS;
		m_ability_map[_T( "砂の力" )] = ABILITY_SANDFORCE;
		m_ability_map[_T( "すり抜け" )] = ABILITY_INFILTRATOR;
		m_ability_map[_T( "スロースタート" )] = ABILITY_SLOWSTART;
		m_ability_map[_T( "力づく" )] = ABILITY_SHEERFORCE;
		m_ability_map[_T( "力持ち" )] = ABILITY_HUGEPOWER;
		m_ability_map[_T( "ヨガパワー" )] = ABILITY_PUREPOWER;
		m_ability_map[_T( "テクニシャン" )] = ABILITY_TECHNICIAN;
		m_ability_map[_T( "適応力 " )] = ABILITY_ADAPTABILITY;
		m_ability_map[_T( "てつのこぶし" )] = ABILITY_IRONFIST;
		m_ability_map[_T( "天然" )] = ABILITY_UNAWARE;
		m_ability_map[_T( "毒暴走" )] = ABILITY_TOXICBOOST;
		m_ability_map[_T( "トランジスタ" )] = ABILITY_TRANSISTOR;
		m_ability_map[_T( "熱暴走" )] = ABILITY_FLAREBOOST;
		m_ability_map[_T( "鋼使い" )] = ABILITY_STEELWORKER;
		m_ability_map[_T( "鋼の精神" )] = ABILITY_STEELYSPIRIT;
		m_ability_map[_T( "張り切り" )] = ABILITY_HUSTLE;
		m_ability_map[_T( "プラス/マイナス" )] = ABILITY_PLUSMINUS;
		m_ability_map[_T( "むしのしらせ" )] = ABILITY_SWARM;
		m_ability_map[_T( "メガランチャー" )] = ABILITY_MEGALAUNCHER;
		m_ability_map[_T( "貰い火" )] = ABILITY_FLASHFIRE;
		m_ability_map[_T( "弱気" )] = ABILITY_DEFEATIST;
		m_ability_map[_T( "龍のあぎと" )] = ABILITY_DRAGONSMAW;
		m_ability_map[_T( "岩運び" )] = ABILITY_ROCKYPAYLOAD;
		m_ability_map[_T( "人でなし" )] = ABILITY_MERCILESS;
		m_ability_map[_T( "古代活性" )] = ABILITY_PROTOSYNTHESIS;
		m_ability_map[_T( "クォークチャージ" )] = ABILITY_QUARKDRIVE;
		m_ability_map[_T( "ヒヒイロの鼓動" )] = ABILITY_ORICHALCUMPULSE;
